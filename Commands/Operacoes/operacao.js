...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    await interaction.editReply({ embeds: [criarEmbedSucesso('Operação agendada', `A operação **${titulo}** foi agendada para ${formatarData(dataObj)}.`)] });

    await registrarLog(client, 'operacao_criada', interaction.user.id, '', `Operação: ${titulo} — ${formatarData(dataObj)}`);
  },
};
